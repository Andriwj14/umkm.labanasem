...
<body>
	...
	<section class="bg-light sec-pad">
		<div class="div-bg" style="background-color:#00AA5B">
			<h4 class="h4-bg text-center"><span class="h4-bg-white">DATA PENGAJUAN</span></h4>
		</div>
		<hr>
    ...
    <thead class="bg" style="background-color:#00AA5B">
       <tr class="text-white">
            ...
            <th class='text-center' width="20%">Nama UMKM</th>
            <th class='text-center' width="15%">Tanggal Pengajuan</th>
			<th class='text-center' width="20%">Deskripsi Pengajuan</th>
		...
		<?php echo "<span style='color:red; font-weight:bold;'>".$pesan."</span>";
			...
			$sql="SELECT l.laporanid as 'kd', r.nama as 'nm',l.tanggal as 'tgl', l.isi as 'isi', l.keterangan as 'keterangan' FROM laporan l INNER JOIN register r ON l.umkmid=r.umkmid WHERE l.kategori='pengajuan' ORDER BY l.dateinsert";
			...
			{			
				...
				while ($rows = mysqli_fetch_assoc($res)) 
				{
					...
					echo "	<td class='text-left'>".html_entity_decode($rows['keterangan'])."</td>";
					if (trim($rows['keterangan'])=='')
					{
						echo "	<td class='text-center'><a class='btn btn-success btn-sm' href='pengajuaneditform.php?lid=$rows[kd]'><span class='glyphicon glyphicon-edit'></span> Beri Komentar</a></td>";
					}
					else
					{
						echo "	<td class='text-center'><a class='btn btn-success btn-sm disabled' href='pengajuaneditform.php?lid=$rows[kd]'><span class='glyphicon glyphicon-edit'></span> Beri Komentar</a></td>";
					}
